<template>
  ...
  >
    <!--    颜色配置器-->
    <ColorPannel @color-config-change="onColorChange" />
    ...
    <div
      class="he-suan-wrap"
      :style="{
        backgroundColor: formColor['engine-color'] || '',
        color: formColor['engine-text-color']
      }"
    >
      <p>保质机</p>
      <p>
      ...
      <p>已过： {{ result }} (小时)</p>
      <p>距离 保质期到期(72h) 剩余进度：： {{ parseInt(72 - result) }}</p>
      <p>
